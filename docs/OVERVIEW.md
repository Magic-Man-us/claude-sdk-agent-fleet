...
One PyPI project (`agent-fleet`), three subpackages, plus one external dependency:

| Package | Holds |
|---------|-------|
| `agent_fleet` (`src/agent_fleet/`) | the pipeline, the router, the pool, and the agent domain models |
| `agent_fleet_api` (`src/agent_fleet_api/`) | FastAPI service — catalog, generate, render, orchestrate, pool routes; installed via the `agent-fleet[api]` extra |
| `agent_fleet_mcp` (`src/agent_fleet_mcp/`) | MCP server exposing the pool as tools (`pool-mcp` console script) |
...
in the root `pyproject.toml`); that repo is public.
...
- recall — [engine/source.py](../src/agent_fleet/engine/source.py): rank every entry by a pluggable `Ranker`, sort by `(-score, id)`, trim to `limit`. `BM25Ranker` (Okapi BM25, normalized to [0,1]) is the default; `TwoStageRanker` narrows by tags first, then reranks.
- select — [engine/select.py](../src/agent_fleet/engine/select.py): drop below `RELEVANCE_THRESHOLD` (unless pinned), equip skills and MCP servers into de-duplicated buckets via `singledispatch`. Tools aren't recalled — every agent gets the fixed `DEFAULT_TOOLS`.
- compose — [engine/compose.py](../src/agent_fleet/engine/compose.py): map the selection into an `AgentSpec`, generating an XML-tagged system prompt unless the caller supplied one.
- score — [engine/efficiency.py](../src/agent_fleet/engine/efficiency.py): three pass/fail dimensions (tool_count, skill_count, prompt_size) against budgets. `EfficiencyReport.passed` is a `computed_field`.
- render — [engine/render.py](../src/agent_fleet/engine/render.py): `render_claude_sdk` → a runnable `claude_agent_sdk` program; `to_options` / `to_agent_definition` / `with_subagents` build the live SDK objects.

## File map

`models/agent/` — the agent domain ([src/agent_fleet/models/agent/](../src/agent_fleet/models/agent/))
- `types.py` — the domain-primitive aliases, the `ModelId`/`AgentEffort` `StrEnum`s, and the default constants (read this first).
- `request.py` — `ProblemRequest` (flat: task · name · tags · team · model · pinned · system_prompt).
...
- [pipeline.py](../src/agent_fleet/engine/pipeline.py) — `assemble()` and `generate()`.
- [source.py](../src/agent_fleet/engine/source.py) · [select.py](../src/agent_fleet/engine/select.py) · [compose.py](../src/agent_fleet/engine/compose.py) · [efficiency.py](../src/agent_fleet/engine/efficiency.py) · [render.py](../src/agent_fleet/engine/render.py) — the five pipeline stages above.
- [pool.py](../src/agent_fleet/engine/pool.py) — `AgentPool` (SQLite) and `AsyncAgentPool` (an `asyncio.to_thread` wrapper for async consumers).
- [dispatch.py](../src/agent_fleet/engine/dispatch.py) — `run_with_capture()`: run an agent live and record the real, resumable session id of the top-level agent *and* every subagent it dispatches, by observing the streamed messages.
- [run.py](../src/agent_fleet/engine/run.py) — `run_agent()`: execute a spec in-process via the SDK's `query()`.
- [orchestrate.py](../src/agent_fleet/engine/orchestrate.py) — `Orchestrator`: reviews the ranked slates, proposes an `AgentSpec`, and spawns it. Review/propose are synchronous and testable without the SDK; only spawn calls into it.
- [acquire_tool.py](../src/agent_fleet/engine/acquire_tool.py) — the `acquire_capability` MCP tool: an agent finds, equips, and runs a capability on demand, mid-run.
- [findings_tool.py](../src/agent_fleet/engine/findings_tool.py) — the `write_finding` MCP tool: a concurrency-safe shared sink for fan-out lens agents.
- [emit.py](../src/agent_fleet/engine/emit.py) — `write_agent()`: source → file on disk. [naming.py](../src/agent_fleet/engine/naming.py) — `slugify_name()`.

`router/` — the deferred-tool capability product
- [capability.py](../src/agent_fleet/router/capability.py) — `CapabilityRouter` + `SkillCard`/`ToolCard`/`McpCard`/`PluginCard`: index the environment once, answer `find_*` with a top-k BM25 slate, load a skill body on demand.
- [mcp_server.py](../src/agent_fleet/router/mcp_server.py) — `FastMCP("skill-router")`; `main()` is the `skill-router` console script.

Also: [settings.py](../src/agent_fleet/settings.py) — `AgentFleetSettings(DiscoverySettings)`, adding the agent/skill directories. [main.py](../src/agent_fleet/main.py) — the CLI.
...
[src/agent_fleet_api/routes.py](../src/agent_fleet_api/routes.py) — `GET
