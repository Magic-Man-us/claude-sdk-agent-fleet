...
def render_claude_sdk(spec: AgentSpec, options: ClaudeAgentOptions | None = None) -> str:
    ...
    return (
        ...
        "from agent_fleet.engine.codex_tool import current_codex_policy, with_codex_tool\n\n\n"
        f"# generated agent: {spec.name}\n"
        f"_BASE_OPTIONS = ClaudeAgentOptions(\n{body}\n)\n"
        "OPTIONS = with_codex_tool(_BASE_OPTIONS, current_codex_policy())\n\n\n"
